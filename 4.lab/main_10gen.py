...
def evaluate_fitness(individual):
    fitness_value = None
    new_fitness_value = banana_function(individual[0],individual[1])
    if fitness_value:
...
def genetic_algorithm(population_size, mutation_chance, max_iterations):
    ...
    print(f"\nMinimum achieved at x = {round(best_individual[0], 4)}; y = {round(best_individual[1], 4)} with f(x, y) = {banana_function(best_individual[0], best_individual[1])}")
